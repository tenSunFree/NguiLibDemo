...
class CentralTractionButton : RadioButton {
    ...
    private fun init() {
        ...
        mR = if (mHeight > mWidth) mHeight / 2 else mWidth / 2
        mr = mR / 2

        // 背景图绘制区域
        mExternalDestRect = Rect(
            (centerx - mr).toInt(), (centery - mr).toInt(),
            (centerx + mr).toInt(),
            (centery + mr).toInt()
        )
        //初始化: 75 75 225 225
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                val mExternalOffesetLimit = mr / 4
                val mInsideOffesetLimit = mr / 3
    ...
    private fun getDistanceTwoPoint(x1: Float, y1: Float, x2: Float, y2: Float): Float {
        return Math.sqrt(
            (Math.pow((x1 - x2).toDouble(), 2.toDouble()) +
                    Math.pow((y1 - y2).toDouble(), 2.toDouble()))
        ).toFloat()
    }
